...
export default function CreatePost() {
  ...
  const deletePostImage = async (e) => {
    ...
    try {
      const response = await axios.post("/deletepostimage", { imagename: previewPic });
      if (response.status === 200) {
        setPreviewPic(null);
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex justify-center px-4 pt-10 pb-4">
      <form className="bg-slate-800 md:px-8 px-4 mb-4 rounded-lg shadow-lg w-full max-w-3xl h-fit" onSubmit={handleSubmit}>
        <h2 className="text-2xl font-semibold text-center text-teal-400 pt-3">
          Create new post
        </h2>
        <div className="w-full h-[1px] bg-gray-700 my-4" />
        <div className="mb-4">
          <label htmlFor="title" className="block mb-2 text-sm font-medium text-sky-800">
            Title
          </label>
          <input
            type="text"
        ...
        <div className="mb-4">
          <label htmlFor="category" className="block mb-2 text-sm font-medium text-sky-800">
            Category
          </label>
          <input
            type="text"
        ...
        <div className="mb-4 h-80">
          <label htmlFor="description" className="block mb-2 text-sm font-medium text-sky-800">
            Summary
          </label>
          <ReactQuill theme="snow" value={description} onChange={setDescription} style={{ height: '15rem' }} />
        </div>

        <div className="mb-4">
          <label htmlFor="photo" className="block mb-2 text-sm font-medium text-sky-800">
            Photo
          </label>
